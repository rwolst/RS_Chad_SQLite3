...
(
    SELECT
	    CASE WHEN (BAT_HOME_ID = 0) THEN AWAY_TEAM_ID ELSE HomeTeam END as BattingTeam,
		CASE WHEN (BAT_HOME_ID = 0) THEN (AWAY_SCORE_CT - HOME_SCORE_CT) ELSE (HOME_SCORE_CT - AWAY_SCORE_CT) END as WinningMargin,
		BAT_HAND_CD,
		INN_CT,
...
Group by BattingTeam, WinningMargin, BAT_HAND_CD, INN_CT;